...
{                                                                   
    ...
    public boolean equalKeys(OrderedDictionary<K,V> dictionary) {
    	if(this.size() != dictionary.size())
    		return false;
    	Iterator<Entry<K,V>> itDic = dictionary.iterator();
    	Iterator<Entry<K,V>> itThis = this.iterator();
    	while(itThis.hasNext() && itDic.hasNext()) {
    		if(itThis.next().getKey().compareTo(itDic.next().getKey()) != 0)
    			return false;
    		}
    	return true;
    	}
    ...
    /*complexidade espacial: O(1)
	  ...
	  - Melhor caso: O(1), ocorre quando as dimensoes dos dicionarios ordenados sao diferentes 
	  - Caso esperado: O(1), porque a probabilidade das chaves serem diferentes sao elevadas
	  - Pior caso: O(n), ocorre quando os dicionarios tem as mesmas chaves	*/	
    
    //Returns the nth entry of the tree inorder traversal
    public Entry<K,V> nthEntry( int n ) throws NoSuchEntryException {
    	if(n > this.size()) 
    		throw new NoSuchEntryException();
    	return nthEntryRec(this.root, n);
    }


	private Entry<K, V> nthEntryRec(BSTNode<K, V> root2, int n) {
		// TODO Auto-generated method stub
		int rootOrder = root.leftSubtreeSize() + 1;
		if(rootOrder == n)
			root.getEntry();
		if(rootOrder < n)
			return nthEntryRec(root.getRight(), n - rootOrder);
		else
			return nthEntryRec(root.getLeft(), n);
	}
	
	/* Melhor Caso (MC) , Caso Esperado (CE) , Pior Caso (PC) 
	  * Complexidade Espacial , MC = O(1) , CE = O(1) , PC = O(1)     
	  * Complexidade Temporal , MC = O(1) , CE = O(log n) , PC = O(n) 
	  * arvore degenerada para a esquerda e estamos a procura do primeiro elemento do percurso , ou 
	  * e uma arvore degenerada para a direita e estamos a procura do ultimo elemento do percurso */
